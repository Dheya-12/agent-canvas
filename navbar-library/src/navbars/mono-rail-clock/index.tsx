...
/**
 * Monospace rail with a live clock.
 *
 * Dark-first, inset from the top edge, with uppercase monospace micro-type
 * and an accent-coloured mark. The trailing edge carries a live clock —
 * minute-aligned rather than drifting, and rendered only after mount so
 * server and client markup agree.
 */
export default function MonoRailClock({ content, className, style, onNavigate }: NavbarProps) {
  const c = useSafeContent(content);
  const { open, onClose, onToggle } = useDisclosure();
  const [openIdx, setOpenIdx] = useState(-1);
  const labels = menuLabels(c);
  const time = useClock(c.clock?.timeZone);

  useScrollLock(open);
  useEscape(open, onClose);
  useCloseOnDesktop(open, onClose, '(min-width: 900px)');
          ...
          <div className={s.tail}>
            ...
            {c.clock && time && (
              <span className={s.clock}>
                <time>{time}</time>
                {c.clock.label && <span className={s.clockLabel}>{c.clock.label}</span>}
              </span>
            )}
            <button
              type="button"
      ...
      <div className={s.panel} data-open={open ? 'true' : 'false'} data-theme={c.theme ?? 'dark'} aria-hidden={!open} dir={c.dir ?? 'ltr'}>
        <nav>
          <ul className={s.panelList}>
            {c.items.map((it, i) => (
              <li key={`p-${i}`} style={{ ['--i' as string]: i }}>
        ...
        <div className={s.panelFoot}>
          ...
          {c.clock && time && <span>{time}{c.clock.label ? ` ${c.clock.label}` : ''}</span>}
        </div>
      </div>
    </>
  );
